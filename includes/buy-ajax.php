...
class YWBuyAjax{
    ...
    {
        add_action('wp_ajax_partner_buy_ajax', [$this, 'partner_buy_func']);
    }
        ...
        if(wp_verify_nonce( $data['_wp_nonce'], 'yw_partner_buy' ) >= 1 ){
            ...
            foreach ($data['buy_quantity'] as $key => $value) {
                if($value != 0 || !empty($value)) {
                    ...
                    $order->add_product($product, $value);
                }
            }
            ...
            if($order->get_item_count() > 0) {
                ...
                $order->add_order_note('سفارش افزوده شده از پنل همکار');

                wp_send_json_success('سفارش شمار ثبت شد. شماره سفارش : ' . $order->get_id());
            }else{
